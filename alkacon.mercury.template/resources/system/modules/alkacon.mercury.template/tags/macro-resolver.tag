...
<%@ attribute name="jspBean" type="org.opencms.jsp.util.CmsJspStandardContextBean" required="false"
    description="The JSP standard context bean to use for the macro resolver. If not provided the macro resolver will not have access to macros for resolving links." %>
...
<%@ variable alias="resolver" name-from-attribute="var" scope="AT_END" declare="true" variable-class="alkacon.mercury.template.CmsExtendedLinkMacroResolver" %>

<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="m" tagdir="/WEB-INF/tags/mercury" %>

<%
    alkacon.mercury.template.CmsExtendedLinkMacroResolver resolver = alkacon.mercury.template.CmsExtendedLinkMacroResolver.newInstance();
    org.opencms.file.CmsObject cms = (org.opencms.file.CmsObject)getJspContext().getAttribute("cms");
    org.opencms.jsp.util.CmsJspStandardContextBean jspBean = (org.opencms.jsp.util.CmsJspStandardContextBean)getJspContext().getAttribute("jspBean");
    if (jspBean != null) {
        resolver.setJspBean(jspBean);
        resolver.setCmsObject(jspBean.getVfs().getCmsObject());
    } else if (cms != null) {
