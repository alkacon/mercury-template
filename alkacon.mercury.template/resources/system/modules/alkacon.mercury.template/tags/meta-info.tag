...
<c:choose>
    <c:when test="${not empty cms.meta.Revisit}">
        <c:set var="pagerevisit"><m:meta-value text="${cms.meta.Revisit}" /></c:set>
    </c:when>
    <c:otherwise>
        <c:set var="pagerevisit">7 days</c:set>
    </c:otherwise>
</c:choose>
...
<meta name="revisit-after" content="${pagerevisit}">
