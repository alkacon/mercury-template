...
/**
 * Form mail sent to the administrator if a registration is cancelled.
 */
public class CmsFormMailCancelAdmin extends A_CmsFormMailAdmin {

    /**
     * Creates a new form mail for the administrator if a registration is cancelled.
    ...
    public void sendMail(Map<String, FileItem> attachements) throws AddressException, EmailException {
        ...
        String mailPrefix = CmsFormMailMessages.get().container(CmsFormMailMessages.MAIL_PREFIX_CANCELLED).key(
    ...
    protected String getMailText() {
        ...
        String mailText = CmsFormMailMessages.get().container(CmsFormMailMessages.INFO_CANCELLED_ADMIN).key(
            m_cms.getRequestContext().getLocale());
        return "<p>" + mailText + "</p>";
    }
